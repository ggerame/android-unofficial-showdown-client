...
- **Native move animations**: damaging and status moves now play Pokémon
  Showdown's actual on-field move animations — particle effects, sprite lunges,
  background flashes and the like — instead of only a hit spark. Around 920 move
  animations plus 45 shared effect routines and 11 status animations are bundled
  in `res/raw/battle_animations.json` (generated by
  `build-tools/build_battle_animations.py`).
- A faithful Kotlin port of Showdown's `BattleScene` coordinate projection
  (`pos()` / `posT()`) and its custom easing curves (ballistic / quadratic /
  swing) drives the timing and trajectories, calibrated to this app's
  battle-field layout. Particles are drawn on `BattleLayout`'s canvas on a
  self-scheduling animation clock.
- The `fx` particle graphics are **fetched at runtime** from
  `play.pokemonshowdown.com/fx/` (and cached by Glide), so nothing copyrighted
...
- **Crash-safe with a graceful fallback**: moves without an animation (or before
  ...
  are caught and logged rather than taking down the battle.
