...
data class TeamValidationResult(val valid: Boolean, val message: String, val rejected: Boolean = false) {
    companion object {
        fun fromPopup(message: String): TeamValidationResult {
            ...
            return TeamValidationResult(accepted && !rejected, message, rejected)
        }
    }
}
