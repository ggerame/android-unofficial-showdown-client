...
class BattleFormatParserTest {
    ...
    @Test fun generationControlsAvailableFields() {
        ...
        val gen2 = FormatProfile.from("gen2ou")
        assertTrue(gen2.hasItems)
        assertFalse(gen2.hasAbilities)
    ...
    @Test fun mirrorsOfficialSpecialModControls() {
        ...
        assertFalse(letsGo.hasHiddenPower)
        assertEquals(50, letsGo.defaultLevel)
        assertTrue(FormatProfile.from("gen7ou").hasHiddenPower)
        assertTrue(FormatProfile.from("gen9nationaldex").hasHiddenPower)
        assertFalse(FormatProfile.from("gen9ou").hasHiddenPower)
        val bdsp = FormatProfile.from("gen8bdspou")
        assertFalse(bdsp.hasDynamax)
        assertFalse(bdsp.hasHiddenPower)
        assertTrue(bdsp.supportsHiddenPower("Unown"))
        assertFalse(bdsp.supportsHiddenPower("Pikachu"))
        assertFalse(FormatProfile.from("gen8ou").supportsHiddenPower("Unown"))
        assertFalse(FormatProfile.from("gen9ou").supportsHiddenPower("Unown"))
